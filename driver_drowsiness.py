...
#             "yawn_stages": []
#         }
#         self.ear_history = deque(maxlen=15)  # Store the last 15 EAR values
#         self.mar_history = deque(maxlen=15)  # Store the last 15 MAR values
...
#     def detect_blink(self, ear):
#         """Improved blink detection using EAR history"""
#         self.ear_history.append(ear)  # Add the current EAR to the history

#         # Calculate the smoothed average EAR
...
#     def detect_yawn(self, mar):
#         """Improved yawn detection using MAR history"""
#         self.mar_history.append(mar)  # Add the current MAR to the history

#         # Calculate the smoothed average MAR
#         smoothed_mar = np.mean(self.mar_history)

#         # Adjust thresholds based on the smoothed MAR
#         if smoothed_mar > self.YAWN_THRESHOLD:
#             self.yawn_frames += 1
#             if self.yawn_frames >= self.YAWN_CONSEC_FRAMES:
#                 if not self.is_yawning:
#                     self.is_yawning = True
#                     self.stats["total_yawns"] += 1
#                     self.stats["yawn_stages"].append("Severe Yawn")
#                     self.play_alarm()
#             return True
#         else:
...
#     def process_frame(self):
#         """Process a single frame with improved blink and yawn detection"""
#         ret, frame = self.cap.read()
#         if not ret:
#             return None, None
...
#                 # Status updates remain unchanged
#                 if ear < self.EYE_ASPECT_RATIO_THRESHOLD and self.COUNTER >= self.EYE_ASPECT_RATIO_CONSEC_FRAMES:
#                     self.status = "SLEEPING !!!"
#                     self.color = (0, 0, 255)
#                     self.play_alarm()
...
#                 # Drawing and text overlays remain unchanged
#                 left_eye_hull = cv2.convexHull(left_eye)
#                 right_eye_hull = cv2.convexHull(right_eye)
#                 mouth_hull = cv2.convexHull(mouth)
#                 cv2.drawContours(face_frame, [left_eye_hull], -1, (0, 255, 0), 1)
...
#                 # Update the frame with EAR and MAR
#                 cv2.putText(frame, f"EAR: {ear:.2f}", (300, 30),
#                             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
#                 cv2.putText(frame, f"MAR: {mar:.2f} - {self.status}", (300, 60),
#                             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
